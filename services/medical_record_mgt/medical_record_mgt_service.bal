...
listener http:Listener medical_record_mgt_service_client = new(9093);
...
service medical_record_mgt_service on medical_record_mgt_service_client {
    ...
    resource function addMedicalRecord(http:Caller caller, http:Request req) {
        ...
        var medicalRecordtReq = req.getJsonPayload();

        if (medicalRecordtReq is json) {
